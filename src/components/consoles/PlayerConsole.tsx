...
export function PlayerConsole() {
    ...
    const handleOnCloseEdit = () => { SetShowEditPlayerModal(false); };
    const handleCloseAdd = () => { SetShowAddPlayerModal(false); };
    const handleCloseGames = () => { setShowGamesByPlayerModal(false); };

    useEffect(() => { loadData(SetPlayerData); }, []);

    const tHeads: string[] = [
        "Player Id", "First Name", "Last Name", "Age", "Gender",
                            ...
                            <tbody>
                                {playerData.map((row, index) => (
                                    <tr key={row.playerId || index}>
                                        ...
                                        <td>
                                            <div className="d-flex gap-2 justify-content-center">
                                                {/* Games button visible to all */}
                                                <Button className="btn-games" onClick={() => handleGetGamesByPlayer(row)}>
                                                    Games
                                                </Button>
